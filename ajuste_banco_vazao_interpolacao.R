...
d2<-ts(vazoes_IL$InterpolacaoLinear)

forecast::autoplot(d2)+theme_minimal()

forecast::ggAcf(d2,lag.max = 100,type = c("correlation"))+labs(y = "FAC Amostral",title="")+
  theme_minimal()
forecast::ggAcf(d2,lag.max = 100,type = c("partial"))+labs(y = "FACP Amostral",title="")+
  theme_minimal()

# plot(density(d2[-test$all.stats$Obs.Num,]))
# a<-boxplot(d2[-test$all.stats$Obs.Num,])
# library(EnvStats)
# test <- rosnerTest(d2, k=300) #k= n?meros de possiveis outliers
# test$all.stats$Obs.Num

# Analise de tendencia deterministica: --------------------------------------- #
randtests::cox.stuart.test(d2,c("two.sided")) #H0: não existe tendencia
trend::cs.test(d2) #H0: não existe tendencia
randtests::runs.test(d2) #H0: não existe tendencia
trend::ww.test(d2) #H0: não existe tendencia
trend::mk.test(d2,continuity = TRUE) #H0: a série é i.i.d. / não existe tendencia
Kendall::MannKendall(d2) #H0: não existe tendencia

# Todos com tendência deterministica, p-valor menor que alpha

# Teste para raiz unitaria: -------------------------------------------------- #
tseries::adf.test(d2,alternative = c("stationary")) #H0: raiz unitária
tseries::pp.test(d2,alternative = c("stationary")) #H0: raiz unitária
tseries::kpss.test(d2, null = c("Level")) #H0: Nivel estac
tseries::kpss.test(d2, null = c("Trend")) #H0: Tend estac

# Sazonalidade --------------------------------------------------------------- #

seastests::kw((d2), freq=365, diff=F, residuals=F, autoarima = T) #H0: Não Sazonal
seastests::fried((d2), freq=365, diff=F, residuals=F, autoarima = T) #H0: Não Sazonal

#p/ o adf e pp estac. e p/ kpss Tem raiz unitaria e tem tendencia deterministica 

# diff.TX<-diff(d2, differences = 1)
...
# a<-(TSA::periodogram(d2))
# 1/a$freq[a$spec==max(a$spec)]
# Mesmo dando 360, acredito que o mais ideal é uma frequência de 365 dias (1 Ano)

dts<-ts(as.vector(d2),start = c(2000,60), frequency = 365)
plot(decompose(dts))
prev<-30
#Separar a serie em treino e teste:
tr<- ts(d2[-((length(d2)-c(prev-1)):length(d2))],start = c(2000,60), frequency = 365) 
te<- ts(d2[(length(d2)-c(prev-1)):length(d2)],start = c(2022,330), frequency = 365)

# test <- EnvStats::rosnerTest(d2, k=12) #k= n?meros de possiveis outliers
# vezoes_mes<- as.data.frame(vezoes_mes) |> 
#   dplyr::mutate(n=1:length(d2)) |> 
...
df4<-data.frame(t3,Y=dts,pred2=c(as.vector(predicao),as.vector(predict.mod$pred)),
...
g4<-ggplot(df4)+
  ...
  geom_line(aes(x=t3,y=pred2),linetype = "dashed",color="red2",size=0.7)+
